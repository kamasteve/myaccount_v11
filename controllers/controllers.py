...
class MyaccountAddons(http.Controller):
    ...
    def execute_query(self):
        ...
        query = "Select (select sum(residual_signed) from account_invoice where date_due < current_date and partner_id=%s) as o,(select sum(residual_signed) from account_invoice where date_due > current_date and partner_id=%s) as a,(select sum(residual_signed) from account_invoice where partner_id=%s ) as b,(select amount from account_payment where partner_id=%s and id=(SELECT MAX(id) FROM account_payment where partner_id=%s and state='posted') ) as c,(select payment_date from account_payment where partner_id=%s and create_date=(SELECT MAX(create_date) FROM account_payment where partner_id=%s and state='posted') ) as d,(select currency_id from account_invoice where partner_id=%s and create_date=(SELECT MAX(create_date) FROM account_payment where partner_id=%s and state='posted')) as e"
        ...
        vars = (user, user, user, user, user, user,user,user,user)
